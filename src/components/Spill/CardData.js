  ...
  return (
    <div className="flex flex-row justify-between font-normal mt-2">
      <div className="flex items-center">
        <img src={userImage} alt="" className="rounded-full h-8 w-8 mr-1.5" />
        <p className="text-sm font-bold">{username}</p>
      </div>
      <div className="flex items-center">
        <div className="flex flex-row items-center mr-2.5">
          <i
            className={`fa-regular fa-message fa-flip-horizontal text-sm -mb-0.5 ${
              commentCount > 0 && "mr-1"
            }`}
          ></i>
          {commentCount > 0 && <p className="text-sm">{commentCount}</p>}
        </div>
        <div className="flex flex-row items-center mr-2.5">
          <i
            className={`fa-solid fa-quote-left ${quoteCount > 0 && "mr-1"}`}
          ></i>
          {quoteCount > 0 && <p className="text-sm">{quoteCount}</p>}
        </div>
        <div className="flex flex-row items-center">
          <svg
            ...
            viewBox="0 0 512 512"
            style={{ fill: "#ffffff" }}
            className={`-mt-0.5 ${likeCount > 0 && "mr-1"}`}
          >
